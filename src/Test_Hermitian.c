 ...
 * @brief QR Algorithm test on a hermitian matrix.
 * @date 2024-11-26
 * 
...
int main(int argc, char **argv) {
    ...
    for(; N1 < N0; ++N1) {
        ...
        for(N2 = 0; N2 < N1; ++N2) {
            R3 = R0 + R2 * (Real) rand() / RAND_MAX;
            R4 = R0 + R2 * (Real) rand() / RAND_MAX;

            Cm0[N2 * N0 + N1] = C_RR_C(R3, R4);
            Cm0[N1 * N0 + N2] = C_RR_C(R3, -R4);
        }
    }

    #ifndef NVERBOSE
    printf("Testing on a hermitian %zu x %zu matrix.\n", N0, N0);
